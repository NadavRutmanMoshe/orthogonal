...
| `tools/feature.js` | **Play's 1024x500 feature graphic**, mandatory and the one asset a screenshot cannot be. The icon's scene drawn wide with the game's name on it, set in the real typefaces out of `css/05-fonts.css`. Three layouts (`feature-A|B|C.png`); `zoom` and `dx` per layout are what keep the type off the cast |
