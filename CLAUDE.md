...
| `tools/icon.js` | **the app icon, drawn**: a voxel scene projected 3D left of a seam and flat right of it, the Rose cube on the seam and a hunter mid-charge beside it. Writes `app/icon/` (1024, 512, a preview sheet) and, with `--android`, the launcher mipmaps. The one place a PNG is checked in, and it is a store asset, not a game asset |
