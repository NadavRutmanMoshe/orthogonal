...
folds. Two things in one silhouette column, twice over, is rule 4, and it is
the boss kill rule, and it is the only way anything in this game dies at
somebody else's hand. (It was four in one column at `x=3`; the owner asked
for one to take each parent at the same moment, and after one version with
the officers abreast, for the walk up to stay on the one line.) The son lives
because
