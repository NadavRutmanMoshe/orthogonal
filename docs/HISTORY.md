...
## The houses were mostly dune, and the ratio nobody had measured

Two things were wrong with the opening's street, and neither was the houses.

**The background was in front of the roofline, not behind it.** Screen height
in this projection is `0.885y - 0.465z`, so two rows further from the camera
is worth almost two courses of height. The dune row at `z=-2`/`z=-3` therefore
lands in exactly the band the walls and the roofs occupy - not under them.
Coloured in outright for a photograph, the dune turned out to be nearly
everything between, beside and behind the two houses: the buildings had no sky
against them anywhere, and the top half of the picture was one warm-brown mass
with a doorway in it. There is no room to move it - pushing the row back to
`z=-5` grows the arena and `recomputeBounds()` frames the arena, so the houses
would simply be drawn smaller. It recedes in VALUE instead, and by far more
than "two steps under the walls" was: near the night sky's own darkness.

**And the tints were under the threshold, which is a number.** The rule in
this file was "roughly half again as much red as green", 1.5, and it is wrong
by measurement. The grass surface is green twice: `#5faa41` over the top half
of the atlas and a `#5aa83f` lip along the top of every side face, both about
**1.8 green to red**. A multiply can only take green away, so a tint under
1.8 leaves every top face and every lip greener than it is warm - which is
why three versions in a row were photographed as brick-coloured walls with a
bright green band along the top of every course. The walls were warm; their
edges were not; and at this camera angle the edges are most of what you see.
Clear 1.8 and the band goes neutral. Clear about **3** and it goes to a warm
line, which is what a course of brick looks like. The hexes look absurdly
orange read on their own - they are not colours, they are filters.

Worth remembering generally: **a tint over one of these surfaces is a ratio
problem, not a colour choice.** Pick the hue you want out the far end, then
check it against the surface's own ratio before believing it.

The rest of the pass was ordinary: the two houses stopped being the same
house twice (different walls, different roofs, the chimney on the other
side), the window holes got a dark pane a row behind them so they read as
windows rather than as gaps to the sky, both doors got a worn track out to the
street, and two small dark houses went up on the dune row so the street does
not end at the second door.
