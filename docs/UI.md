...
| `95-home.css` | `.home` overlay, `.hcont` (CONTINUE, `--sec`), `.hshop`, `.hward` / `.hward.hmine` (WARDROBE, MY LEVELS), `.tiny` (SETTINGS) and `.tiny.hmulti` + `.hlock` (the locked MULTIPLAYER under it), `body.athome`. **Type: title 34px, doors 14.5px, `.tiny` 12px and capped there by its absolutely-positioned gamepad rather than by its box** (16px doors were tried and rejected as too big; the room is there, at 327px nothing overflows until 18.5px).** CONTINUE's second line is `levelShort()` |
| `96-desk.css` | **the computer**: the key strip, the key cap that stands in for the ghost hand, the `.padfocus` ring, Settings > Keys, **hover on every button** and the corner buttons' `data-tip` (written by `deskTips()`), and - under `@media (min-aspect-ratio:5/4)` - **the wide layouts**: home as a grid (menu left, stand right, no markup change), the wardrobe with `.wbody{display:contents}` so the panel grid places list/caption left and the case right, Settings as a six-column grid (cards span 2, nav rows 3, a lone nav row the whole width), Keys as two columns. Panels are targeted by `#panel[data-kind]`, which `showPanel()` writes. Every rule is behind `body.desk` or a class only `js/26-desk.js` writes. Sizes are phone sizes - the page is zoomed on a big screen. Shoot it with `--pc` (1920x1080) or `--desktop --desk` (the Deck) |
| `97-textsize.css` | **not a screen**: the LARGE half of Menu > Text size. Every rule is prefixed `body.tx-large`, medium writes nothing, and the numbers are written out per declaration rather than multiplied - a box only has to grow enough to hold its own contents |
...
| Menu | `#panel` | `menuPanel()` (`16-panels.js`); rows via `seg()`. **WORLDS is the first thing in the body** - a `.psec`/`.pworlds` row into `sectionPicker()`, above the back-to-this-shelf row. **How it plays** is one card of three rows - Controls, Level size, Fights speed - because the age card writes all three together. Every card heading and both More rows carry a `.pci` glyph (`panelIcon()`, `PANEL_ICONS`). Under 360px `.crow` stacks its label above its options: the label column is **98px** to hold "Fights speed" - it was 92px, measured before `05-fonts.css` bundled the real IBM Plex Mono, which is wider and made that label wrap; 361px and up is the case it serves, since below that the label stacks | `80-panel-tall` (`.psec`, `.pworlds`, `.pcard>h4 .pci`), `40-panels` (`.srow`), `50-layout-cues` (`.crow .seg`, the 360px query) | `menu` |
...
| Offer cards (stars, refill, out of lives) | `#panel.offer` | `offerShell(kick,title,lead,acts,note,tone,actClass)` via `starsOffer`, `hintRefillOffer`, `struggleOffer`. **The out-of-lives card passes `pair`**, which turns the stacked `.ma` into the win card's side-by-side row: TRY AGAIN in the goal's green with `#bRetry`'s own circular arrow (`retryIcon()`), and where NEXT LEVEL stands on that card, SKIP over WATCH AN AD. It wraps to a stack on a narrow phone, like `.wonrow` | `85-map` (`.panel.offer .okick .olead`, `.ma .go .ad .qt .mn`, `.ma.pair .oai .two`) | `starsoffer`, `refill`, `struggle` |
