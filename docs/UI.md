...
| `96-desk.css` | **the computer**: the key strip, the key cap that stands in for the ghost hand, the `.padfocus` ring, Settings > Keys. Every rule is behind `body.desk` or a class only `js/26-desk.js` writes. Sizes are phone sizes - the page is zoomed on a big screen. Shoot it with `--pc` (1920x1080) or `--desktop --desk` (the Deck) |
